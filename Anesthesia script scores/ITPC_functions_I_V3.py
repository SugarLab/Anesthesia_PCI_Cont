...
## Top level function
def itpc_for_paper(data, fs, lowest_frequency = 8, highest_frequency=13, 
                 b1 = -0.25, b2 = -0.05, n_straps = 500, alpha = 0.05, 
                 itpc_post_x1=0, itpc_post_x2=0.5, fH_start=8, fH_end=40, n=15):
    """
    Compute convolution for estimates of significant power and Inter-Trial 
    ...
    ----------
    data : 3D np.array
        input data (samples x channels x trials).
    fs : float
        Sampling rate of the data.
    lowest_frequency : int, optional
        Lowest frequency for the wavelet convolution. The default is 5.
    highest_frequency : int, optional
        Highest frequency for the wavelet convolution. The default is 40.
    b1 : float (s), optional
        Beginning of baseline. The default is -0.25.
    b2 : float (s), optional
        End of baseline. The default is -0.05.
    n_straps : int, optional
        Number of bootstrap samples to draw when estimating maximum value of 
        baseline. The default is 500.
    alpha : float, optional
        percentile to use when finding estimated maximum in baseline across 
        bootstrap resamples. The default is 0.05.
    itpc_post_x1 : float, optional
        First timepoint to consider for significant ITPC. The default is 0.
    itpc_post_x2 : float, optional
        Last timepoint to consider for significant ITPC. The default is 0.5.
    fH_start : int, optional
        lower limit of High Frequency range for calculating mean power 
        (frequency must be present in frequencies array). The default is 8.
    fH_end : TYPE, optional
        Upper limit of High Frequency range for calculating mean power 
        (frequency must be present in frequencies array). TThe default is 40.

    Returns
    -------
    conv : 4d np array
        The result of wavelet convolution.
    frequencies : 1d np array
        the frequencies at which the convolution is computed.
    decibels : 4d np array
        the log power normalized by baseline (within frequencies).
    itpc : TYPE
        DESCRIPTION.
    decibels_m_sig : TYPE
        DESCRIPTION.
    ...
    # compute wavelet convolution for full data
    conv, fs, x1, x2, frequencies, time_data, wavelet_family = wavelet_convolution(
        data, 
        lowest_frequency=lowest_frequency, 
        highest_frequency=highest_frequency, 
        num=int(highest_frequency - lowest_frequency + 1), 
        fs=fs
    )
    conv_last_n_trials = conv[:,:,-n:,:]
    conv_first_n_trials = conv[:,:,:n,:]
    conv_except_n_trials = conv[:,:,n:-n,:]
    
    # compute itpc for full data
    phases, itpc = extract_phase_calculate_itpc(conv)
    
    # get significant itpc for full data
    t_max_percentile_itpc = bootstrap_stat_itpc(
        phases, fs, x1, b1=b1, b2=b2, n_straps=n_straps, alpha=alpha
    )
    itpc_sig = threshold_itpc(t_max_percentile_itpc, itpc)
    
    # get time for drop of itpc for full data
    itpc_d, itpc_d_m, itpc_fr = itpc_drop(
        itpc_sig, time_data, fs, frequencies, 
        itpc_post_x1, itpc_post_x2, fH_start, fH_end
    )
    

    # compute itpc for all except n trials of data
    phases, itpc = extract_phase_calculate_itpc(conv_except_n_trials)
    
    # get significant itpc for all except n trials of data
    t_max_percentile_itpc = bootstrap_stat_itpc(
        phases, fs, x1, b1=b1, b2=b2, n_straps=n_straps, alpha=alpha
    )
    itpc_sig = threshold_itpc(t_max_percentile_itpc, itpc)
    
    # get time for drop of itpc for all except n trials of data
    between_n_itpc_d, itpc_d_m, itpc_fr = itpc_drop(
        itpc_sig, time_data, fs, frequencies, 
        itpc_post_x1, itpc_post_x2, fH_start, fH_end
    )
    
  
    # compute itpc for last n trials of data
    phases, itpc = extract_phase_calculate_itpc(conv_last_n_trials)
    
    # get significant itpc for last n trials of data
    t_max_percentile_itpc = bootstrap_stat_itpc(
        phases, fs, x1, b1=b1, b2=b2, n_straps=n_straps, alpha=alpha
    )
    itpc_sig = threshold_itpc(t_max_percentile_itpc, itpc)
    
    # get time for drop of itpc for last n trials of data
    last_n_itpc_d, itpc_d_m, itpc_fr = itpc_drop(
        itpc_sig, time_data, fs, frequencies, 
        itpc_post_x1, itpc_post_x2, fH_start, fH_end
    )
    
    # compute itpc for first n trials of data
    phases, itpc = extract_phase_calculate_itpc(conv_first_n_trials)
    
    # get significant itpc for first n trials of data
    t_max_percentile_itpc = bootstrap_stat_itpc(
        phases, fs, x1, b1=b1, b2=b2, n_straps=n_straps, alpha=alpha
    )
    itpc_sig = threshold_itpc(t_max_percentile_itpc, itpc)
    
    # get time for drop of itpc for first n trials of data
    first_n_itpc_d, itpc_d_m, itpc_fr = itpc_drop(
        itpc_sig, time_data, fs, frequencies, 
        itpc_post_x1, itpc_post_x2, fH_start, fH_end
    )
    
    ### Compute GMFP for full data and first and last n trials
    ...
    return itpc_d, between_n_itpc_d, last_n_itpc_d, first_n_itpc_d, gmfp, last_n_gmfp, first_n_gmfp 


## Top level function
def compute_itpc(data, fs, lowest_frequency = 5, highest_frequency=40, 
                 b1 = -0.25, b2 = -0.05, n_straps = 500, alpha = 0.05, 
                 itpc_post_x1=0, itpc_post_x2=0.5, fH_start=8, fH_end=40):
    """
    Compute convolution for estimates of significant power and Inter-Trial 
    Phase Clustering (ITPC).
    ...
    powers_norm, decibels, decibels_m = extract_power_normalization(
        conv, fs, x1, x2, b1=b1, b2=b2
    )
    
    phases, itpc = extract_phase_calculate_itpc(conv)
    
    t_max_percentile_db, t_min_percentile_db = bootstrap_stat_db(
        powers_norm, fs, x1, b1=b1, b2=b2, n_straps=n_straps, alpha=alpha
    )
    ...
    decibels_m_sig, itpc_sig = threshold_db_itpc(
        decibels_m, t_max_percentile_itpc, t_min_percentile_db, 
        t_max_percentile_db, itpc
    )
    ...
    return conv, frequencies, decibels, itpc, decibels_m_sig, itpc_sig, itpc_d
...
def threshold_itpc (T_max_percentile_ITPC, av_vector_length):
...
def itpc_drop(data1, time_data, fs, frequencies, ITPC_post_x1 = 0, ITPC_post_x2 = 0.8, fH_start = 8, fH_end =40, last_sig=False):
    """
    returns npy arrays:
    ...
    last_sig = False       --> indicate whether to estimate drop based on last significant clustering (otherwise use first drop to zero after significant clustering)    
    ...
    #% IF no phase clustering between 0 and .8s (end) -> No synchrony at all -> set to first sample (0s)
        ...
        #% FIND the last significant ITPC in the range      
            ...
            #% FIND the first non-significant ITPC after significant ITPC in the range
            ITPC_index_lastsig[ch] = len(ITPC_postim_av_H[:,ch]) - 1
            for i, itpc_val in enumerate(ITPC_postim_av_H[:-1,ch]):
                if itpc_val>0 and ITPC_postim_av_H[i+1,ch]==0:
                    ITPC_index_lastsig[ch] = i
                    break
                
    
    print(time_postim.shape, flush=True)
    print(ITPC_index_lastsig,flush=True)
