...
for subject in subject_folders:
    results_table_ACE = {'subject': subject}
    ...
    for file in data_files:
        if file.endswith(file_extension) and condition in file:
            ...
            try:
                ...
                results_table_ACE[condition_name] = resultACE

            except Exception as e:
                print(f"Error processing {file}: {e}")
    
    # After all files for this subject are processed, append the results
    results_list.append(results_table_ACE)

results_df = pd.DataFrame(results_list)

# Save the DataFrame to an Excel file
excel_filename_ACE = 'ACE_scores.xlsx'
excel_filepath_ACE = f'{output_dir}/{excel_filename_ACE}'

# Save the dataframe to Excel and insert the saved image
with pd.ExcelWriter(excel_filepath_ACE, engine='openpyxl') as writer:
    results_df.to_excel(writer, sheet_name='Sheet1', index=False)

print(f"Table saved to {excel_filepath_ACE}")
