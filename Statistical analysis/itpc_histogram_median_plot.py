...
plt.hist(aggregated_data_1, bins=num_bins, range=hist_range, alpha=0.7, color='blue')
...
plt.figure(figsize=(12, 6))
plt.hist(aggregated_data_3, bins=num_bins, range=hist_range, alpha=0.7, color='red')
plt.axvline(x=median_value_3, color='black', linestyle='--', label=f'Median (Group 3: Experience): {median_value_3:.2f}')
plt.title('Histogram of ITPC drop values for "Experience" (red) and "No experience" (blue)')
plt.xlabel('ITPC drop value (s)')
plt.ylabel('Channel Count')
plt.ylim(0, common_max_count)  # Set common y-axis limit
plt.legend()
