...
for subject in subject_folders:
    ...
    # Skip subject if output folder already exists
    outpath = os.path.join(output_dir, subject)
    if os.path.exists(outpath):
        print(f"Output for {subject} already exists, skipping.")
        continue
    ...
    for file in data_files:
        ...
        if file.endswith(file_extension) and condition in file:
            ...
            try: 
                os.mkdir(outpath)
                print('Path created for', subject)
            except:
                print("Path exists for", subject)
